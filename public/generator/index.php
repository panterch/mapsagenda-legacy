...
if(mktime(0, 0, 0, $monat,$tag,  $jahr)< mktime(0, 0, 0, $_POST[date10_month],$_POST[date10_date],  $_POST[date10_year]) && mktime(0, 0, 0, $monat,$tag,  $jahr) > mktime(0, 0, 0, $_POST[date9_month],$_POST[date9_date],  $_POST[date9_year])){
...
function cleanme($input){ // Zeichen formatieren f¸r HTML ausgabe
$output=htmlspecialchars($input);
$output=ereg_replace("Ñ","\"",$output);

$output=ereg_replace("ì","\"",$output);
...
<DIV ID="testdiv1" STYLE="position:absolute;visibility:hidden;background-color:white;layer-background-color:white;"></DIV></td>
	...
	?>
      <p class="untertitel">2. Eintr&auml;ge w√§hlen:  </p>
...
<tr>
          <td width="23%" class="kleintitel"><? echo substr($val,8,2).".". substr($val,5,2).".".substr($val,0,4) ?></td>
          <td colspan="2" class="kleintitel"><? echo $getxml->title[0] ?></td>
          <td width="9%" align="right" class="kleintitel">
            <input type="checkbox" name="xmllist[]" id="<? echo strlen($getxml->desc[0].$getxml->location[0]) ?>" value="<? echo $val ?>" class="formular" onChange="zaehlen()"/>
         </td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td colspan="2" valign="top" style="padding-left:5px">√úbersetzungen: <?
          	$count=0;
			$gruen="";
			$rot="";
...
?><span class="gruen"  onmouseover="Tip('<span class=klein><? echo cleanme($getxml->location[0])?></span>',DELAY, 0,BGCOLOR, '#FFFFFF', WIDTH,200)" onmouseout="UnTip()">vorhanden</span><?
 }else{ ?>
 ...
 /*
            Schwerpunkt: 
            <input type="checkbox" name="checkbox" id="checkbox" class="formular"/>
